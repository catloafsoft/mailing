...
1. Install the Netlify NextJS plugin: `pnpm add @netlify/plugin-nextjs`
...
3. In the Netlify deploy settings, set the "build command" to `pnpm install && pnpm exec mailing server build` and set "publish directory" to `.mailing/.next`
