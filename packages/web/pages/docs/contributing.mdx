...
`pnpm dev` starts the cli in dev mode
...
- Create a new next app in your projects directory by running `pnpm create next-app --typescript` for a typescript app OR `pnpm create next-app` for a js app
- In the next app, run `yalc add mailing`, this creates `node_modules/mailing` and `node_modules/.bin/mailing`. (Note: built-in link commands often do not add the bin file, which is why `yalc` is prefered)
- Make your changes in `mailing`
- Run `pnpm run build` in the `mailing` root directory to create new `dist` files
...
- In the project root, run `pnpm test:e2e` to run the full framework test suite, including cypress and jest tests for all supported frameworks.
...
- `--skip-build` to skip the pnpm build part of the script, useful when debugging something unrelated to the build
